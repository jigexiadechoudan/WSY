...
def load_json_data():
    json_path = os.path.join(os.path.dirname(__file__), "../../../剪纸.json")
    with open(json_path, "r", encoding="utf-8") as f:
        return json.load(f)


def import_paper_cut():
    print("Starting paper cut data import...")

    data = load_json_data()
    basic = data.get("基本信息", {})
    name = basic.get("名称", "剪纸")
    world_heritage = basic.get("世界非遗", {})
    world_flag = "是" if world_heritage.get("入选") else "否"

    main_query = """
    MERGE (paper:Heritage {name: $name, type: "Craft"})
    SET paper.aliases = $aliases,
        paper.非遗级别 = $level,
        paper.入选时间 = $time,
        paper.非遗编号 = $number,
        paper.类别 = $category,
        paper.世界非遗 = $world_heritage
    RETURN paper
    """
    ...
    if display:
        ...
        CREATE (paper)-[:HAS_PERFORMANCE]->(perf)
        RETURN perf
        """
    ...
    print("[OK] Paper cut data imported successfully!")
    return True
