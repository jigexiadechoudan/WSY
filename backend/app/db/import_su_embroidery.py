...
def load_json_data():
    json_path = os.path.join(os.path.dirname(__file__), "../../../苏绣.json")
    with open(json_path, "r", encoding="utf-8") as f:
        return json.load(f)


def import_su_embroidery():
    print("Starting su embroidery data import...")

    data = load_json_data()
    basic = data.get("基本信息", {})
    name = basic.get("名称", "苏绣")
    ...
    print("[OK] Su embroidery data imported successfully!")


if __name__ == "__main__":
    try:
        import_su_embroidery()
    finally:
        neo4j_service.close()
